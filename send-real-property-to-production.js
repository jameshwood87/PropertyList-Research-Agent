...
const realPropertyData = {
  ...
  // Don't start analysis - just preview
  previewOnly: true
};

async function sendToProduction() {
  console.log('🚀 Sending real property to production...\n');
  ...
  try {
    const response = await makeRequest('/api/property-analysis', 'POST', realPropertyData);
    
    console.log('\n✅ Successfully sent to production!');
    ...
    console.log('⚠️  Note: Analysis will not start automatically (preview only)');
    
  } catch (error) {
    console.log('❌ Error sending to production:', error.message);
  }
}
...
// Run the script
sendToProduction().catch(console.error); 